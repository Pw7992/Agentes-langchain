...
class PreguntaInput(BaseModel):
    pregunta: str = Field(description="Pregunta o instrucción del usuario en lenguaje natural")
    
def crear_herramientas(df):
    herramienta_informacion_df = Tool(
        name = 'Informaciones_DF',
        func = lambda pregunta: informacion_df.run({"pregunta":pregunta, "df":df}),
        args_schema=PreguntaInput,
        description = """
...
# Ya es una herramienta completa - solo le damos un nombre y descripción propios
    herramienta_codigos_python = PythonAstREPLTool(locals={"df": df})
    herramienta_codigos_python.name = "Herramienta_Códigos_Python"
    ...
    return [herramienta_informacion_df, herramienta_resumen_estadistico, herramienta_generar_grafico, herramienta_codigos_python]
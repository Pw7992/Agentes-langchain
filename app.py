...
if archivo_cargado:
    ...
    # Prompt react (formato tool-calling, sin parseo manual de texto)
    ...
    prompt_react_es = ChatPromptTemplate.from_messages([
        ("system", """Eres un asistente que responde en castellano.
 
                        Tienes acceso a un dataframe pandas llamado `df`.
                        Aquí están las primeras filas del dataframe, obtenidas usando `df.head().to_markdown()`:
                        {df_head}
                        
                        Responde a las preguntas del usuario de la mejor manera posible, usando las herramientas
                        disponibles cuando sea necesario. Sé claro, organizado y responde siempre en castellano."""),
        ("human", "{input}"),
        MessagesPlaceholder("agent_scratchpad"),
    ])
 
    prompt_react_es = prompt_react_es.partial(df_head=df_head)

 
    # Agente
    agente = create_tool_calling_agent(llm=llm, tools=tools, prompt=prompt_react_es)
    ...
    if st.button("Reporte de Informaciones Generales", key="boton_reporte_general"):
        with st.spinner("Generando Reporte 🕵️‍♂️..."):
            respuesta = orquestador.invoke({"input": "Quiero un reporte con informaciones generales sobre los datos"})
    ...
    if st.button("Reporte de estadísticas descriptivas", key="boton_reporte_estadisticas"):
        with st.spinner("Generando Reporte 🕵️‍♂️..."):
            respuesta = orquestador.invoke({"input": "Quiero un Reporte de estatísticas descriptivas"})
    ...
    pregunta_sobre_datos = st.text_input("Realiza una pregunta sobre los datos (ej: 'cual es el promedio de tiempo de entrega?')")
